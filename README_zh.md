...
- **播放器模式**：像看视频一样捕获句子，支持 Space 播放/暂停、S 捕获、↑/↓ 切换句子、F 全屏；捕获后立即保留字幕、截图和音频，可跨不同视频组合句子
...
- **播放器模式**：打开 `/player`，选择视频并加载字幕或转录结果，播放时按 `S` 捕获当前句；按 `↑` / `↓` 在字幕句子间跳转，按 `F` 进入全屏。捕获队列会即时冻结截图和音频，生成前无需重新上传原视频。
